...
then
    ...
    then
        ...
        echo "Merging done"
        python $DIR/../tools/funcs.py --func extract_error --input $COMPILE_INFO/$RESULT_FOLDER/deepseek.parquet --output $COMPILE_INFO/$RESULT_FOLDER/deepseek_compile_error.parquet
        echo "Extract errors done"
    else
        if [ "$PHASE" == "cr" ]
        then
            python $DIR/../tools/funcs.py --func cr --input $COMPILE_INFO/$RESULT_FOLDER/deepseek_compile_error.parquet
        else
            exit 0
        fi
    fi
