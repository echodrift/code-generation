...
i=1
while [ $i -le $concurency ]; do
    screen -dmS hardhat$i bash -c "python compile.py -i $DIR/data/batch$i.parquet -hh hardhat$i -o $DIR/out/result$i.parquet -e $error -p $DIR/error/error$i.parquet"
    i=$(( i + 1))
done
