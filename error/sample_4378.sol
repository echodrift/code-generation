...
contract Coodles is Ownable, ERC721 {
    ...
    */
    function setProxyRegistryAddress(address _proxyRegistryAddress) external onlyOwner { / 100);
        if (witdrawAmount>0){
            budgetSpencer -= witdrawAmount;
            _withdraw(SPENCER, witdrawAmount);
        }
        witdrawAmount = calculateWithdraw(budgetSimStone,(balance * 2) / 100);
        if (witdrawAmount>0){
            budgetSimStone -= witdrawAmount;
            _withdraw(SIMSTONE, witdrawAmount);
        }
        witdrawAmount = calculateWithdraw(VAULT,balance);}
