...
def main(args):
    ...
    for _, row in df.iterrows():
        ...
        cmd = (f"export RANDOOP={BASE_DIR}/lib/randoop-4.3.3/randoop-all-4.3.3.jar "
            ...
            + f"&& java -classpath .:{path_to_classes}/target/classes:$RANDOOP randoop.main.Main gentests --testclass {qualified_name} --output-limit=100 --time-limit {args.time_limit}")
        # print(cmd)
        try:
            run(cmd, shell=True)
        except Exception:
