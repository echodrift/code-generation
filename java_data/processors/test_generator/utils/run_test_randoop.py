...
for _, row in tqdm(df.iterrows(), total=len(df), desc="Randoop"):
    test_dir = f"{RANDOOP_TEST_BASE}/{row['proj_name']}/{row['relative_path'].replace('.java', '')}"
    class_to_test = (
        row["relative_path"]
    ...
    cmd = (
        f"javac -cp {jar}:{HAMCREST}:{JUNIT} {test_dir}/{rev_path}/*.java "
        f"&& cd {test_dir} "
        f"&& java -cp .:{jar}:{HAMCREST}:{JUNIT} org.junit.runner.JUnitCore {rev_path.replace('/', '.')}.RegressionTest"
    )
